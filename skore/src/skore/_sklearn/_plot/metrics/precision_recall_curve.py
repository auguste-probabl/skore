...
class PrecisionRecallCurveDisplay(_ClassifierCurveDisplayMixin, DisplayMixin):
    ...
    @property
    def lines_(self) -> list[Line2D]:
        if isinstance(self.ax_, np.ndarray):
            return [line for ax in self.ax_.flat for line in ax.get_lines()]
    ...
    def _plot_matplotlib(
        ...
        *,
        ...
        if self.report_type == "estimator":
            ...
            self.ax_, info_pos_label = self._plot_cross_validated_estimator(
                estimator_name=(
                    self.precision_recall["estimator_name"].cat.categories.item()
                    if estimator_name is None
                    else estimator_name
            ...
            self.ax_, info_pos_label = self._plot_comparison_estimator(
                estimator_names=self.precision_recall["estimator_name"].cat.categories,
                pr_curve_kwargs=pr_curve_kwargs,
            )
        elif self.report_type == "comparison-cross-validation":
            self.ax_, info_pos_label = self._plot_comparison_cross_validation(
                estimator_names=self.precision_recall["estimator_name"].cat.categories,
                pr_curve_kwargs=pr_curve_kwargs,
            )
        else:
